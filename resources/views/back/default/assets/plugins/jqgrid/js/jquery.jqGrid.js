		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(ci){
				...
				if(this.search){
					...
					{
					case "select":
						surl = this.surl || soptions.daturlBack;
						if(surl) {
...
// Form Functions
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		function setAttributes(elm, atr, exl ) {
			var exclude = ['dataInit','dataEvents','daturlBack', 'buildSelect','sopt', 'searchhidden', 'defaultValue', 'attr', 'custom_element', 'custom_value'];
		...
		switch (eltype)
		{
			...
			case "select" :
				...
				if(options.daturlBack !== undefined) {
					...
					$.ajax($.extend({
						url: $.isFunction(options.daturlBack) ? options.daturlBack.call($t, rowid, vl, String(options.name)) : options.daturlBack,
						type : "GET",
						dataType: "html",
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
						...
						if(cm[i].editable===true) {
							...
							$.jgrid.bindEv.call($t, elc, opt);
							//Again IE
							if(cm[i].edittype === "select" && cm[i].editoptions!==undefined && cm[i].editoptions.multiple===true  && cm[i].editoptions.daturlBack===undefined && $.jgrid.msie) {
								$(elc).width($(elc).width());
							}
							cnt++;
						}

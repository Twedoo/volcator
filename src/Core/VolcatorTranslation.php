...
class VolcatorTranslation
{
    ...
    {
        return array_merge(VolcatorFile::getFilesResources(
            array_diff(scandir(
    ...
    public static function getFiles($module, $language, $file)
    {
        if ($module == 'langRresources') {
            ...
            $data = array_merge(
                VolcatorFile::getFilesResources(array_diff(scandir($globalPath, 1), ['..', '.', 'validation.php']), $language),
                VolcatorFile::getFilerByType($globalPath, 'php', ['..', '.', 'validation.php'])
            );
        } else {
            $globalPath = app_path() . '/Modules/' . $module . '/lang/' . $language . '/' . $module;
            $data = VolcatorFile::getFilerByType($globalPath, 'php', ['..', '.', 'validation.php']);
        }
        return $data;
    }

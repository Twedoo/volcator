...
import { editPageApplication, deleteApplication } from "@/components/service/service-volcator";
...
export function createDesigner(vueInstance) {
    ...
    return {
        ...
        initDesigner(resetFormJson) {
            ...
            //输出版本信息和语雀链接
            console.info(
                `%Volcator %cVer${VARIANT_FORM_VERSION} %chttps://volcator.com/v1/documentation`,
                "color:##7575A3;font-size: 22px;font-weight:bolder",
                "color:##232135;font-size: 12px",
                "color:#333"
            );
            ...
            this.formConfig.gateCheckAuthorized = true;
        },
         ...
         * @param application
         */
        deleteApplicationAndClearDesigner(application) {
            deleteApplication(application).then(response => {
        ...
        containerDroppableHighlight() {
            this.formConfig.droppableButton = !this.getContainerDroppableHighlight();
        },

        getContainerDroppableHighlight() {
            return this.formConfig.droppableButton;
        },

        previewOrRender() {
            return this.formConfig.previewOrRender;
        },

        setPreviewOrRender(status) {
            this.formConfig.previewOrRender = status;
        },
        ...
        copyNewContainerWidget(origin) {
            ...
            if (newCon.type === "basic-horizontal") {
                let newCol = deepClone(this.getContainerByType("header-item"));
                let tmpId = generateId();
                newCol.id = "header-item-" + tmpId;
                newCol.options.name = "headerItem" + tmpId;
                newCon.widgetList.push(newCol);
                //
                newCol = deepClone(this.getContainerByType("main-item"));
                tmpId = generateId();
                newCol.id = "main-item-" + tmpId;
                newCol.options.name = "mainItem" + tmpId;
                newCon.widgetList.push(newCol);
            } else if (newCon.type === "grid") {
        ...
        saveFormContentToStorage() {
            ...
            let applicationId = window.localStorage.getItem("volcator_application");

            if (this.formConfig.pageId && applicationId && this.formConfig.gateCheckAuthorized === false) {
                editPageApplication({

...
export default defineConfig({
  ...
  resolve: {
    alias: {
        "@": resolve(__dirname, './Views/Vue/src'), // 路径别名
    },
    extensions: ['.js', '.vue', '.json', '.ts'] // 使用路径别名时想要省略的后缀名，可以自己 增减
  },

  optimizeDeps: {
    include: ['./Views/Vue/lib/vuedraggable/dist/vuedraggable.umd.js', 'quill'],
  },
  ...
  css: {
    preprocessorOptions: {
      scss: {
        /* 自动引入全局scss文件 */
        additionalData: '@import "./Views/Vue/src/styles/global.scss";'
      }
    }
  },
  ...
  build: {
    //minify: false,
    lib: {
      entry: resolve(__dirname, './Views/Vue/install.js'),
      name: 'VFormDesigner',
